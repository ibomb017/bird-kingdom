...
struct ExpenseController: RouteCollection {
    ...
    func getExpenses(req: Request) async throws -> [ExpenseDTO] {
        ...
        // 查询所有涉及的用户昵称，用于情侣共享时显示创建者
        let allUserIds = Set(expenses.map { $0.userId })
        var userNameMap: [Int64: String] = [:]
        if allUserIds.count > 1 {
            let users = try await User.query(on: req.db)
                .filter(\.$id ~~ Array(allUserIds))
                .all()
            for u in users {
                if let uid = u.id {
                    userNameMap[uid] = u.nickname ?? "用户\(uid)"
                }
            }
        }
        
        return expenses.map { ExpenseDTO.from($0, creatorName: userNameMap[$0.userId]) }
    }
    ...
    func getExpenseStats(req: Request) async throws -> ExpenseStatsDTO {
        ...
        for expense in expenses {
            let monthKey = dateFormatter.string(from: expense.expenseDate)
            monthlyMap[monthKey, default: 0] += expense.amount
        }
        
        let sortedMonthlyStats = monthlyMap
            .sorted { $0.key > $1.key }
            .prefix(6)
            .map { MonthlyExpenseStat(month: $0.key, amount: $0.value) }
        
        return ExpenseStatsDTO(
            totalExpense: totalAmount,
            monthlyExpense: monthlyAmount,
            expenseByCategory: categoryStats,
            monthlyStats: sortedMonthlyStats.isEmpty ? nil : sortedMonthlyStats
        )
    }
    ...
    func addExpense(req: Request) async throws -> ExpenseDTO {
        ...
        struct CreateExpenseRequest: Content {
            ...
            let birdId: Int64?
            let birdName: String?
            let expenseDate: String?  // 改为 String 类型，前端发送 "yyyy-MM-dd" 格式
        }
        ...
        let parsedDate = input.expenseDate.flatMap { dateFormatter.date(from: $0) } ?? Date()
    ...
    func updateExpense(req: Request) async throws -> ExpenseDTO {
        ...
        // 允许清空备注、鸟儿关联（直接赋值而非 if let）
        expense.note = input.note
        expense.birdName = input.birdName
        expense.birdId = input.birdId
        // 解析日期字符串
        if let expenseDateStr = input.expenseDate {
            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = "yyyy-MM-dd"
            if let parsedDate = dateFormatter.date(from: expenseDateStr) {
