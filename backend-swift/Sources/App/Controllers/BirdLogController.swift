...
struct BirdLogController: RouteCollection {
    ...
    func getAllLogs(req: Request) async throws -> [BirdLogDTO] {
        ...
        let birdIds = allBirds.compactMap { $0.id }
        
        // 构建 birdId -> birdName 映射（包含共享鸟）
    ...
    func getLogById(req: Request) async throws -> BirdLogDTO {
        ...
        guard hasAccess || bird.userId == userId else {
            throw Abort(.forbidden, reason: "无权查看该日志")
        }
        ...
        return BirdLogDTO.from(log, birdName: bird.nickname, imageUrls: imageUrls.isEmpty ? nil : imageUrls)
    }
    ...
    func getLogsByBirdId(req: Request) async throws -> [BirdLogDTO] {
        ...
        // 检查用户是否有权查看该鸟的日志（包括伴侣和共享用户）
        let hasAccess = try await checkBirdEditAccess(userId: userId, birdId: birdId, on: req.db)
        guard hasAccess || bird.userId == userId else {
            throw Abort(.forbidden, reason: "无权查看该鸟的日志")
        }
    ...
    func getWeightTrend(req: Request) async throws -> [WeightTrendDTO] {
        ...
        if let birdId = birdId {
            ...
            // 验证权限：自己的鸟 或 伴侣的鸟
            guard bird.userId == userId || bird.userId == partnerUserId else {
                throw Abort(.forbidden, reason: "无权查看该鸟的体重趋势")
            }
